  ...
  files.forEach((pkgJsonFileRaw) => {
    ...
    if (isWorkspacePackage(pkgName)) {
      if ((pkgJson?.scripts?.dev || pkgJson?.scripts?.build) && !pkgJson?.scripts?.["build-dev"]) {
        errors.push({
          msg: `Required "build-dev" script not found ${pkgJsonFile}! Whenever a "dev" or "build" script is declared in a package.json, there must be a "build-dev" script as well.`,
        });
      }
